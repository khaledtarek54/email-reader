...
}(function ($) 
{
	...
	DateTimePicker.prototype = {
		...
		init: function () 
		{
			...
			//alert($(oDTP.element).data('parentelement') + " -- " + $(oDTP.element).attr('data-parentelement'));
		...
		{
			var oDTP = e.data.obj;
			var currentId = $(oDTP.element).attr('id');
			currentId = currentId.replace('dtBox_','');
			var datedesignType = $('#'+currentId).attr('design_type');
			if(datedesignType == 'datetime' || datedesignType == 'date')
			{
				return false;
			}
		...
		_setButtonAction: function(bFromTab)
		{
			...
			{
				//alert('2');
				//alert(oDTP._setOutput());
				var actualDateTime = oDTP._setOutput();

				if($('.datePickerTimeZone').length >=1 )
				{
					if($('.datePickerTimeZone').val() !='')
					{
						var timZoneHour = $('.datePickerTimeZone').val();
						timZoneHour = timZoneHour.replace(':00','');
						timZoneHour = parseInt(timZoneHour);
						//alert(parseInt(timZoneHour));
						let dt = new Date(oDTP._setOutput());
		         		dt.setHours(dt.getHours() - timZoneHour);

		         		//var logeedTimeZone = $('#currentAddEditUserTimeZone').val();
		         		var logeedTimeZone = $('#clock2').html();

		         		var logeedTimeZone = logeedTimeZone.substring(
						    logeedTimeZone.indexOf("(") + 1, 
						    logeedTimeZone.lastIndexOf(")")
						);
		         		//alert(logeedTimeZone);
		         		logeedTimeZone = logeedTimeZone.replace(':00','');
						logeedTimeZone = parseInt(logeedTimeZone);
						
		         		dt.setHours(dt.getHours() + logeedTimeZone);

		         		oDTP.oData.dCurrentDate = new Date(dt);
		         	}
		         	var currentId = $(oDTP.element).attr('id');
					currentId = currentId.replace('dtBox_','');
					//$('#'+currentId).attr('value', oDTP._setOutput());
					// alert(currentId);
	         		//alert(cGMT);
				}
		...
		_modifyPicker: function()
		{
			...
			if(oDTP.settings.showHeader)
			{
				...
				//alert(this.attr('design_type'));
				// $(oDTP.element)
				// console.log('test by nn');
				// console.log($(oDTP.element).attr('id'));
				// console.log('tfffffffffff');
				var currentId = $(oDTP.element).attr('id');
				if(currentId !='dtBox')
				{
					currentId = currentId.replace('dtBox_','');
					var datedesignType = $('#'+currentId).attr('design_type');
					//console.log(datedesignType);
					if(datedesignType == 'datetime')
					{
						var logeedUserCountry = $('#clock2').html();
						var options = "<option value=''>Select time zone</option>";
						var selectedOption = " selected='selected' ";
						$('.syTimeZone').each(function(e){
							var countryCode = $(this).attr('country_code');
							countryCode = countryCode.replace(' ','');
							//console.log(logeedUserCountry+' == '+countryCode);
							if(logeedUserCountry == countryCode)
							{
							...
							options+="<option value='"+$(this).attr('timezone')+"'  "+selectedOption+" >"+$(this).attr('country')+'</option>';
						});
						//sHeader += "<select class='datePickerTimeZone'>"+options+"</select><span class='dtpicker-timezone'>Convert</span>";
						sHeader += "<select class='datePickerTimeZone' style='height: 32px;border-radius: 20px !important;'>"+options+"</select>";
		   				sHeader += "<span class='dtpicker-timezone' style='margin: 10px;padding: 10px;color: #fff;cursor: pointer;background-color: blue;border-radius: 31px !important;display:none;'>Convert</span>";

						sHeader += "<br/><span class='timezoneDate'></span>";
		...
		_addEventHandlersForPicker: function()
		{
			...
			$(oDTP.element).find(".dtpicker-timezone").click(function(e)
			{
				//alert(oDTP._setOutput());
				// $('.timezoneDate').html(oDTP._setOutput());
				//alert($('.datePickerTimeZone').val());
				var timZoneHour = $('.datePickerTimeZone').val();
				timZoneHour = timZoneHour.replace(':00','');
				timZoneHour = parseInt(timZoneHour);
				//alert(parseInt(timZoneHour));
			...
			oDTP._setButtons();
         		...
         		// 	newDay = '0'+newDay;
         		// }
         		// var newHours = dt.getHours();
         		// if(newHours < 10)
         		// {
         		// 	newHours = '0'+newHours;
         		// }
         		// var timeAfterZone = dt.getFullYear()+'-'+newMonth+'-'+newDay+' '+newHours+':00';
         		//$('.timezoneDate').html(timeAfterZone);
         		//$('.timezoneDate').html(dt);
				// if(oDTP.settings.buttonClicked)
